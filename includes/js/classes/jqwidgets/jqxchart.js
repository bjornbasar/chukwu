	...
	a.extend(a.jqx._jqxChart.prototype, {
		...
		_render : function(v) {
			...
			var M = {
				x : P.left,
				y : P.top,
				width : v.width - P.left - P.right,
				height : v.height - P.top - P.bottom
			};
			this._paddedRect = M;
			...
			var C = this.renderer.createClipRect({
				x : b.x,
				y : b.y,
				width : b.width,
				height : b.height
		...
		_renderChartLegend : function(E, c, d, h) {
			...
			for ( var w = 0; w < E.length; w++) {
				...
				var o = E[w].text;
				var p = this.renderer.measureText(o, 0, {
					"class" : n
				});
				if (p.height > j) {
					j = p.height
				...
				if (!d && q.x + l + p.width < c.x + c.width && q.y + k + p.height < c.y + c.height) {
					...
					this.renderer.attr(B, {
						...
						stroke : v,
						"stroke-width" : 1,
						"stroke-opacity" : E[w].opacity
					});
					this.renderer.text(o, q.x + l + 1.5 * u, q.y + k, p.width, j, 0, {
						"class" : n
					}, false, "center", "center");
					this.renderer.endGroup();
		...
		_renderCategoryAxis : function(e, w, L, d) {
			...
			var u = r.text;
			var f = {
				visible : (r.showGridLines != false),
				color : (r.gridLinesColor || "#888888"),
				unitInterval : (r.gridLinesInterval || r.unitInterval),
				dashStyle : r.gridLinesDashStyle
			};
			var z = {
				visible : (r.showTickMarks != false),
				color : (r.tickMarksColor || "#888888"),
				unitInterval : (r.tickMarksInterval || r.unitInterval),
				dashStyle : r.tickMarksDashStyle
			};
			...
			var M = {
				text : r.description,
				style : I,
				halign : r.horizontalDescriptionAlignment || "center",
				valign : r.verticalDescriptionAlignment || "center",
				textRotationAngle : P ? -90 : 0
			};
			var h = {
				textRotationAngle : p,
				style : v,
				halign : c,
				valign : r.verticalTextAlignment || "center",
				textRotationPoint : r.textRotationPoint || "auto",
			...
			return this._renderAxis(P, D, M, h, {
				x : w.x,
				y : w.y,
				width : w.width,
				height : w.height
			}, d, F, false, O, A, f, z, L, t)
		},
		...
		_renderSpiderAxis : function(e, G) {
			...
			var f = {
				visible : (B.showGridLines != false),
				color : (B.gridLinesColor || "#888888"),
				unitInterval : (B.gridLinesInterval || B.unitInterval),
				dashStyle : B.gridLinesDashStyle
			};
			var I = {
				visible : (B.showTickMarks != false),
				color : (B.tickMarksColor || "#888888"),
				unitInterval : (B.tickMarksInterval || B.unitInterval),
				dashStyle : B.tickMarksDashStyle
			};
			var W = {
				text : B.description,
				style : S,
				halign : B.horizontalDescriptionAlignment || "center",
				valign : B.verticalDescriptionAlignment || "center",
			...
			var k = {
				...
				style : F,
				halign : d,
				valign : B.verticalTextAlignment || "center",
				textRotationPoint : B.textRotationPoint || "auto",
		...
		_renderSpiderValueAxis : function(e, c) {
			...
			var f = {
				visible : (D.showGridLines != false),
				color : (D.gridLinesColor || "#888888"),
				unitInterval : (D.gridLinesInterval || h || 1),
				dashStyle : D.gridLinesDashStyle
			};
			var b = {
				stroke : f.color,
				fill : "none",
			...
			var w = {
				visible : (D.showTickMarks != false),
				color : (D.tickMarksColor || "#888888"),
				unitInterval : (D.tickMarksInterval || h),
				dashStyle : D.tickMarksDashStyle
			};
			if (w.visible) {
				tickMarkSize = 5;
				var b = {
		...
		_renderAxis : function(N, J, Z, q, D, c, L, p, ab, H, e, E, Y, d) {
			...
			if (Z.text != undefined && Z != "") {
				...
				var g = this.renderer.measureText(Z.text, v, {
					"class" : Z.style
				});
				u.width = g.width;
				u.height = g.height;
				if (!Y) {
					this.renderer.text(Z.text, D.x + (N ? (!J ? V : -V + 2 * D.width - u.width) : 0), D.y + (!N ? (!J ? D.height - V - u.height : V) : 0), N ? u.width : D.width, !N ? u.height : D.height, v, {
						"class" : Z.style
			...
			for ( var W = 0; W < z.length; W++, S += b) {
				var C = z[W].text;
				var g = this.renderer.measureText(C, q.textRotationAngle, {
					"class" : q.style
				});
				if (g.width > R) {
					R = g.width
		...
		_renderValueAxis : function(o, h, l, p) {
			...
			var v = {
				text : j.description,
				style : q,
				halign : j.horizontalDescriptionAlignment || "center",
				valign : j.verticalDescriptionAlignment || "center",
				textRotationAngle : e ? 0 : (!this.rtl ? -90 : 90)
			};
			var z = j.itemsClass;
			if (!z) {
			...
			var d = {
				style : z,
				halign : j.horizontalTextAlignment || "center",
				valign : j.verticalTextAlignment || "center",
				textRotationAngle : j.textRotationAngle || 0,
				textRotationPoint : j.textRotationPoint || "auto",
				textOffset : j.textOffset
			};
			...
			var L = {
				visible : (j.showGridLines != false),
				color : (j.gridLinesColor || "#888888"),
				unitInterval : M,
				dashStyle : j.gridLinesDashStyle
			};
			...
			var s = {
				visible : (j.showTickMarks != false),
				color : (j.tickMarksColor || "#888888"),
				unitInterval : t,
				dashStyle : j.tickMarksDashStyle
			};
		...
		_animatePieSlices : function(w, c) {
			...
			for ( var B = 0; B < b.length; B++) {
				...
				var r = this._showLabel(w.groupIndex, w.serieIndex, G.itemIndex, {
					x : 0,
					y : 0,
					width : 0,
					height : 0
				...
				G.labelElement = this._showLabel(w.groupIndex, w.serieIndex, G.itemIndex, {
					x : m,
					y : k,
					width : r.width,
					height : r.height
			...
			for ( var d = 0; d < f.length; d++) {
				...
				if (!c) {
					...
					this.renderer.attr(c, {
						...
						"stroke-dasharray" : l["stroke-dasharray"]
					});
					k.element = c
				}
				if (this._isVML) {
				...
				k.labelElement = this._showLabel(l.groupIndex, l.seriesIndex, k.itemIndex, {
					x : h - b,
					y : g - b,
					width : 2 * b,
					height : 2 * b
		...
		_showToolTip : function(m, k, E, z, c) {
			...
			if (g) {
				...
				if (this._toolTipElement.vLine && this._toolTipElement.hLine) {
					...
					this._toolTipElement.vLine = this.renderer.line(C, this._plotRect.y, C, this._plotRect.y + this._plotRect.height, {
						stroke : A,
						"stroke-width" : this.crosshairsLineWidth || 1,
						"stroke-dasharray" : this.crosshairsDashStyle || ""
					});
					this._toolTipElement.hLine = this.renderer.line(this._plotRect.x, B, this._plotRect.x + this._plotRect.width, B, {
						stroke : A,
						"stroke-width" : this.crosshairsLineWidth || 1,
						"stroke-dasharray" : this.crosshairsDashStyle || ""
					})
		...
		_animateLine : function(s, m) {
			...
			var f = s.settings;
			if (!s.pathElement) {
				s.pathElement = this.renderer.path(g, {
					"stroke-width" : f.stroke,
					stroke : f.colors.lineColor,
					"stroke-opacity" : f.opacity,
					"fill-opacity" : f.opacity,
					"stroke-dasharray" : f.dashStyle,
					fill : s.isArea ? f.colors.fillColor : "none"
				});
				this._installHandlers(s.pathElement, s.groupIndex, s.serieIndex)
			} else {
				this.renderer.attr(s.pathElement, {
					d : g
				})
			}
			if (s.labelElements) {
			...
			if (s.pointsArray.length == o.length) {
				...
				if (e != "none") {
					...
					for ( var j = 0; j < o.length; j++) {
						...
						var n = this._showLabel(s.groupIndex, s.serieIndex, s.pointsArray[j].index, {
							x : q.x,
							y : q.y,
							width : 0,
							height : 0
		...
		_select : function(d, i, h, c) {
			...
			this.renderer.attr(d, {
				stroke : b.lineColorSelected,
				fill : b.fillColorSelected,
				"stroke-width" : e.stroke + 0
			})
		},
		_unselect : function() {
			if (this._selected) {
		...
		_getSeriesColorsInternal : function(u, h) {
			...
			return {
				...
				fillColorSelected : q,
				lineColorSymbol : n,
				lineColorSymbolSelected : b,
				fillColorSymbol : o,
				fillColorSymbolSelected : c
			}
		},
		...
		_getBezierPoints : function(h) {
			...
			if (m.length < 3) {
				...
				for ( var g = 0; g < m.length - 1; g++) {
					...
					var l = g == 0 ? 81 : k;
					var f = {
						x : ((-c[0].x + l * c[1].x + c[2].x) / l),
						y : ((-c[0].y + l * c[1].y + c[2].y) / l)
					};
					if (g == 0) {
						l = k
					}
					var d = {
						x : ((c[1].x + l * c[2].x - c[3].x) / l),
						y : ((c[1].y + l * c[2].y - c[3].y) / l)
					};
					e.push({
						x : c[1].x,
						y : c[1].y
